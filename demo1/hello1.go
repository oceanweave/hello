...
// 因此建议开发项目时，只创建一个 go.mod, 对于子目录中的包，采用【项目名（go.mod 名称，也就是 repo 地址，github.com/{username}/{repo-name}】+【包名(一般来说就是所在目录名称）】形式引用，也就是之前说的解法3，详情查看 demo3 文件夹和 main.go 对其的引用
// 【版本号形式】会报错，报错信息如下：
